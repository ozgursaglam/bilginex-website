...
function brevoSubscribe_blog() {
  var email = document.getElementById('brevo-email-blog').value.trim();
  var btn = document.getElementById('brevo-btn-blog');
  var successEl = document.getElementById('brevo-success-blog');
  var errorEl = document.getElementById('brevo-error-blog');
  var formArea = document.getElementById('brevo-form-blog');
...
document.getElementById('brevo-email-blog').addEventListener('keypress',function(e){if(e.key==='Enter')brevoSubscribe_blog();});
...
document.querySelectorAll('.kat-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.querySelectorAll('.blog-kart').forEach(kart => {
      if (kat === 'Tümü' || kart.dataset.kat === kat) {
    ...
    document.getElementById('blogBos').style.display = gorunen === 0 ? 'block' : 'none';
  });
});
