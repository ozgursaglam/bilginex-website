...
function brevoSubscribe_dijital_donusum() {
  var email = document.getElementById('brevo-email-dijital_donusum').value.trim();
  var btn = document.getElementById('brevo-btn-dijital_donusum');
  var successEl = document.getElementById('brevo-success-dijital_donusum');
  var errorEl = document.getElementById('brevo-error-dijital_donusum');
  var formArea = document.getElementById('brevo-form-dijital_donusum');
...
document.getElementById('brevo-email-dijital_donusum').addEventListener('keypress',function(e){if(e.key==='Enter')brevoSubscribe_dijital_donusum();});
...
(function(){
  ...
  steps.forEach(function(s){
    s.style.cssText += ";opacity:0;transform:translateY(28px);transition:opacity 0.6s ease,transform 0.6s ease";
  });
...
(function(){
  function alignBenefits(){
    var list = document.querySelector('.benefits-list');
    var img = document.querySelector('.benefits-img img');
    if(!list || !img) return;
    var h = list.offsetHeight;
    if(h > 0) img.style.height = h + 'px';
  }
  window.addEventListener('load', alignBenefits);
  window.addEventListener('resize', alignBenefits);
})();
